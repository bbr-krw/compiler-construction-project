...
enum BytecodeSignatures : uint8_t {
    ...
    // ioperands: none
    // stack operands: iterator, target
    // pushes modified iterator (iterator + 1 in case if iterator <= target, iterator - 1 otherwise)
    // used by range-based loops
    BC_RNGSPC = 28,

    // ioperands: none
    // stack operands: iterable (array/string/tuple)
    // returns length of popped object
    BC_LENGTH = 29,
